...
<p align="center">
  ...
  客户端 · 路由规则 · 上游 —— 三模块解耦，支持一对多路由与负载策略。
</p>
...
auth.middleware          → 识别 req.client
        ↓
routes/route.middleware  → 选择 route → target → upstream → key
        ↓
proxy.service            → 转发到上游
        ↓
返回上游响应
...
- **Target**：路由目标，包含 `upstreamId` 和 `upstreamModel`
- **Upstream**：上游 API 端点，包含 `baseUrl` 和一组 `keys`
- **Key**：上游 API 的鉴权密钥，归属于某个 Upstream
...
登录后台，进入「上游」页面，编辑默认上游或新增上游，填入你自己的 `baseUrl` 和密钥。
